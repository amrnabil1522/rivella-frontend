...
export const MyContextProvider = ({ children }) => {
  const baseURL = `${process.env.REACT_APP_BACKEND_URL}/api/v1/rivella`;
  ...
  const addToWishlist = async (chalet) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 409) {
        ...
        error.response &&
        error.response.data.message === "invaild token"
      ) {
        Swal.fire({
          icon: "error",
          title: "يجب عليك تسجيل الدخول اولا",
  ...
  const removeFromWishlist = async (chalet) => {
    ...
    try {
      await axiosInstance.delete(`/wishlist/${chalet._id}`, {
        headers: { token },
      });
